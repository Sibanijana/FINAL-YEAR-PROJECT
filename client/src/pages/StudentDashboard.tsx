...
import { Calendar, Clock, BookOpen, FileText, GraduationCap, Bell, Star, TrendingUp } from "lucide-react";
...
const StudentDashboard = () => {
  const upcomingClasses = [
    { subject: "Mathematics", time: "09:00 - 10:30", room: "A-101", instructor: "Dr. Smith", type: "Lecture" },
    { subject: "Physics Lab", time: "14:00 - 16:00", room: "Lab-B", instructor: "Prof. Johnson", type: "Practical" },
    { subject: "Chemistry", time: "16:30 - 18:00", room: "C-301", instructor: "Dr. Brown", type: "Lecture" },
  ];

  const assignments = [
    { subject: "Mathematics", title: "Calculus Problem Set", dueDate: "Tomorrow", progress: 75, urgent: true },
    { subject: "Physics", title: "Lab Report - Motion", dueDate: "3 days", progress: 45, urgent: false },
    { subject: "Chemistry", title: "Organic Compounds Essay", dueDate: "1 week", progress: 20, urgent: false },
  ];
            ...
            <CardContent>
              <div className="space-y-4">
                {upcomingClasses.map((class_, index) => (
                  <div key={index} className="flex items-center justify-between p-4 rounded-lg border border-slate-200 hover:border-purple-300 transition-colors">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-slate-500 mt-1">Instructor: {class_.instructor}</p>
                    </div>
                ...
                {grades.map((grade, index) => (
                  <div key={index} className="p-3 rounded-lg border border-slate-200">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="font-medium text-slate-900">{grade.subject}</h3>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-slate-600 min-w-[3rem]">{grade.percentage}%</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {assignments.map((assignment, index) => (
                <div key={index} className="p-4 rounded-lg border border-slate-200 hover:border-purple-300 transition-colors">
